...
N           = nrow(dental.wide)
n           = nrow(dental.long)
resid       = residuals(nlme.randslope,type="response")
scaledResid = numeric(n);
scaledDep   = numeric(n)
for ( i in 1:N ) {
  ## Get covariance matrix for subject i; take cholesky decomposition
  Sigma_i  = getVarCov(nlme.randslope, individual = i, type = 'marginal')
  L_i      = t( chol(Sigma_i[[1]]) )

  ## Obtain residual for ith individual
  di  <- dental.long[which(dental.long$id==i),]
  x   <- cbind((di$gender=="F")*1.0,(di$gender=="M")*1.0,(di$gender=="F")*di$age,(di$gender=="M")*di$age)    
  resid_i  = di$distance - x%*%c(unlist(coef(nlme.randslope)[i,1:4]))


  if (i==1) print(Sigma_i[[1]])
  if (i==1) print(L_i)  
  if (i==1) print(resid_i)
    
  ## Compute scaled residual
  scaledResid[dental.long$id == i] = solve(L_i) %*% resid_i
  
  if (i==1) print(scaledResid[dental.long$id == i])
  
  scaledDep[dental.long$id == i]   = solve(L_i) %*% dental.long[dental.long$id == i,]$distance
}
#print(cbind(dental.long$id,dental.long$age,scaledDep,scaledResid))
plot(scaledDep,scaledResid)
```
...
Note that the implementation performs one-step influence diagnostics
...
### Analysis Code using LME4 package
```{r, warning = F, message = F}
library(lme4)
library(lmerTest)

lmm <- lmerTest::lmer(distance ~ 0 + gender + age:gender + (1 + age | id), data = dental.long, REML = TRUE)
summary(lmm)
```

### Test a contrast of the parameters using KR method
```{r, warning = F, message = F}
lmerTest::contest(lmm,c(0,0,1,-1),ddf=c("Kenward-Roger"))
```


### Compute summaries for beta using KR method
```{r, warning = F, message = F}
drop1(lmm,ddf=c("Kenward-Roger"))
```


### Compute predictions accounting for uncertainty
```{r, warning = F, message = F}
eBlup <- predict(lmm, newdata = dental.long)
eBlup
```
